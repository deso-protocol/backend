...
func (fes *APIServer) GetLatestNFTDropEntry() (_dropEntry *NFTDropEntry, _err error) {
	...
	_, vals, err := fes.GlobalState.GlobalStateSeek(seekKey, seekKey, maxKeyLen, 1, true, true)
	if err != nil {
...
func (fes *APIServer) GetNFTDropEntry(nftDropNumber uint64) (_dropEntry *NFTDropEntry, _err error) {
	...
	dropEntryBytes, err := fes.GlobalState.GlobalStateGet(keyBytes)
	if err != nil {
...
func (fes *APIServer) AdminUpdateNFTDrop(ww http.ResponseWriter, req *http.Request) {
	...
	err = fes.GlobalState.GlobalStatePut(globalStateKey, updatedDropEntryBuf.Bytes())
	if err != nil {
