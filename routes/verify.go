...
func (fes *APIServer) getPhoneNumberMetadataFromGlobalState(phoneNumber string) (_phoneNumberMetadata *PhoneNumberMetadata, _err error) {
	...
	phoneNumberMetadataBytes, err := fes.GlobalState.GlobalStateGet(dbKey)
	if err != nil {
		return nil, errors.Wrap(fmt.Errorf(
...
func (fes *APIServer) putPhoneNumberMetadataInGlobalState(phoneNumberMetadata *PhoneNumberMetadata) (_err error) {
	...
	err = fes.GlobalState.GlobalStatePut(dbKey, metadataDataBuf.Bytes())
	if err != nil {
		return errors.Wrap(fmt.Errorf(
...
func (fes *APIServer) JumioCallback(ww http.ResponseWriter, req *http.Request) {
	...
	if val, _ := fes.GlobalState.GlobalStateGet(uniqueJumioKey); val == nil || userMetadata.RedoJumio {
		...
		if err = fes.GlobalState.GlobalStatePut(uniqueJumioKey, []byte{1}); err != nil {
...
func (fes *APIServer) JumioVerifiedHandler(userMetadata *UserMetadata, jumioTransactionId string, publicKeyBytes []byte, utxoView *lib.UtxoView) (_userMetadata *UserMetadata, err error) {
	...
	if desoNanos := fes.GetJumioDeSoNanos(); desoNanos > 0 || userMetadata.ReferralHashBase58Check != "" {
		...
		if userMetadata.ReferralHashBase58Check != "" && payReferrer {
			...
			if err = fes.GlobalState.GlobalStatePut(pkidReferralHashRefereePKIDKey, []byte{1}); err != nil {
			...
			if err = fes.GlobalState.GlobalStatePut(tstampPKIDReferralHashRefereePKIDKey, []byte{1}); err != nil {
				glog.Errorf("JumioVerifiedHandler: Error adding to the index of users who were referred by a given referral code")
			}
...
func (fes *APIServer) GetJumioDeSoNanos() uint64 {
	val, err := fes.GlobalState.GlobalStateGet(GlobalStateKeyForJumioDeSoNanos())
	if err != nil {
		return 0
	}
