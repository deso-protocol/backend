...
func (fes *APIServer) WyreWalletOrderSubscription(ww http.ResponseWriter, req *http.Request) {
	...
	if err := fes.GlobalState.GlobalStatePut(GlobalStateKeyForWyreOrderID(orderIdBytes), []byte{1}); err != nil {
	...
	if transferId != "" {
		...
		if btcPurchased > 0 {
			...
			if val, _ := fes.GlobalState.GlobalStateGet(wyreOrderIdKey); val == nil {
				// Mark this order as paid out
				if err = fes.GlobalState.GlobalStatePut(wyreOrderIdKey, []byte{1}); err != nil {
				...
				if err != nil {
					...
					if err = fes.GlobalState.GlobalStateDelete(wyreOrderIdKey); err != nil {
...
func (fes *APIServer) UpdateWyreGlobalState(ww http.ResponseWriter, publicKeyBytes []byte, timestampNanos uint64, wyreWalletOrderMetadata WyreWalletOrderMetadata) {
	...
	if err := fes.GlobalState.GlobalStatePut(globalStateKey, wyreWalletOrderMetadataBuf.Bytes()); err != nil {
...
func (fes *APIServer) GetWyreWalletOrderMetadataFromGlobalState(publicKey string, timestamp uint64) (*WyreWalletOrderMetadata, error) {
	...
	currentWyreWalletOrderMetadataBytes, err := fes.GlobalState.GlobalStateGet(globalStateKey)
	if err != nil {
		return nil, err
	}
...
func (fes *APIServer) GetWyreWalletOrdersForPublicKey(ww http.ResponseWriter, req *http.Request) {
	...
	_, values, err = fes.GlobalState.GlobalStateSeek(prefix, validPrefix, maxKeyLen, 100, true, true)
	if err != nil {
