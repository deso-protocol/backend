...
func (fes *APIServer) AdminPinPost(ww http.ResponseWriter, req *http.Request) {
	...
	if requestData.UnpinPost {
		err = fes.GlobalState.GlobalStateDelete(dbKey)
		if err != nil {
		...
		err = fes.GlobalState.GlobalStatePut(dbKey, []byte{1})
		if err != nil {
...
func (fes *APIServer) AdminRemoveNilPosts(ww http.ResponseWriter, req *http.Request) {
	...
	keys, _, err := fes.GlobalState.GlobalStateSeek(
	...
	for _, dbKeyBytes := range keys {
		...
		// from global state
		if postEntry == nil {
			err = fes.GlobalState.GlobalStateDelete(dbKeyBytes)
			if err != nil {
				_AddBadRequestError(ww, fmt.Sprintf(
