...
func (fes *APIServer) _getRepostPostEntryResponse(postEntry *lib.PostEntry, addGlobalFeedBool bool, utxoView *lib.UtxoView, readerPK []byte, maxDepth uint8) (_repostPostEntry *PostEntryResponse, err error) {
	...
	if postEntry.RepostedPostHash != nil {
		...
		if repostedPostEntry == nil {
			...
			repostedPostEntryResponse, err = fes._postEntryToResponse(repostedPostEntry, addGlobalFeedBool, utxoView, readerPK, maxDepth-1)
			if err != nil {
...
func (fes *APIServer) _postEntryToResponse(postEntry *lib.PostEntry, addGlobalFeedBool bool, utxoView *lib.UtxoView, readerPK []byte, maxDepth uint8) (
	*PostEntryResponse, error) {
	...
	if stakeIDStr == "" {
		...
		repostPostEntryResponse, _ = fes._getRepostPostEntryResponse(postEntry, addGlobalFeedBool, utxoView, readerPK, maxDepth)
	}
	...
	for _, filteredPubKey := range filteredPubKeys {
		...
		for _, repostPostEntry := range repostPostEntries {
			repostPostEntryResponse, err := fes._postEntryToResponse(
				repostPostEntry, false, utxoView, readerPublicKeyBytes, 2)
			if err != nil {
